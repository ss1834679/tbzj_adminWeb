  ...
  <div id="cityOperation_idx">
    ...
    <el-card class="box-card" shadow="never">
      <div slot="header" class="clearfix">
        <span class="title">装修公司列表</span>
        <el-button plain size="small" @click="addCityOperation" icon="el-icon-plus">添加装修公司</el-button>
      </div>
      <div class="content">
        <div class="search">
          ...
          <div class="search_box">
            <el-input placeholder="请输入公司名称" v-model="searchInput" @keydown.native.enter="selectByKwd" @blur="selectByKwd"></el-input>
          </div>
          <div class="search_box">
            <el-input
              placeholder="请输入联系人"
        ...
        <el-table :data="CompanySelectList" border style="width: 100%" row-key="id" @selection-change="getSelection" ref="cityoperationLists">
          ...
          <el-table-column prop="isDisplay" label="显示" align="center">
            <template slot-scope="scope">
              <switch-com :boolen="scope.row.isDisplay" activeText="显示" inactiveText="隐藏" @changeValue="(value)=>{changeStatus(scope.row.id)}"></switch-com>
            </template>
          </el-table-column>
...
import { trimCompanySelectList, trimCompanyDelete, trimCompanyVerify } from "../../api/cityOperation";
export default {
  components: {
    switchCom,
    paginationCom,
  ...
  data() {
    return {
      ...
      searchTime: [] /* 搜索时间过滤 */,
      startTime: null /* 搜索开始时间过滤 */,
      endTime: null /* 搜索结束时间过滤 */,
      batchValue: "" /* 批量操作的类型 */,
      options: [
  ...
  methods: {
    ...
    changeStatus(id) {
      ...
      trimCompanyVerify({ companyIds: this.batchArr }).then(data => {
    ...
    cityOperationEdit(rows) {
      this.$router.push({ name: "decorate_edit", query: { id: rows.id } });
    },
    // 添加装修公司
    addCityOperation() {
      this.$router.push({ name: "decorate_add" });
    },
    /* 批量操作 判断是什么操作 */
    batchOperation() {
      if (this.batchValue == "delete") {
        /* 批量删除 */
        this.$confirm("此操作将删除这些装修公司, 是否继续?", "提示", {
    ...
    deleteArticel(rows) {
      ...
      this.$confirm("此操作将删除该装修公司, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
    ...
    /* 渲染表格 */
    setView(data) {
      this.CompanySelectList = data.data.data.list;
    ...
    // 获取数据列表
    showAll(val) {
      trimCompanySelectList({ params: { pageNo: val ? val : 1, pageSize: 20 }, data: {} }).then(data => {
        if (data.data.status == 0) {
          this.setView(data);
        }
      });
