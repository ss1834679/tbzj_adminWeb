    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="add-content" v-if="typeshow">
        <el-form :model="freightForm" ref="freightForm" label-width="130px">
          ...
          <el-form-item label="不配送区域：" prop="phone">
            <add-temp
              :templateType="freightForm.templates.type"
              :type="3"
          ...
          <el-form-item>
            <el-button type="primary" @click="submitForm('freightForm')">提交</el-button>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
...
import { updateTemplates, getTemplatesById } from "../../api/setings";
export default {
  data() {
    return {
      typeshow: true,
  ...
  mounted() {
    getTemplatesById(this.$route.query.id).then(data => {
      if (data.data.status == 200) {
        this.freightForm = data.data.content;
        this.refreshTable();
      }
    });
  },
  methods: {
    ...
    submitForm() {
      ...
      // let valid = true;
      if (valid) {
        updateTemplates(this.freightForm).then(data => {
          if (data.data.status == 200) {
