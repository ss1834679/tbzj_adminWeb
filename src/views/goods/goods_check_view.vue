<template>
  <div id="good_veiw">
    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="content">
        <el-form label-width="100px" class="viewGood">
          ...
          <el-form-item class="buttomItem">
            <el-button @click="checkGood()">审核</el-button>
          </el-form-item>
        </el-form>
...
import { getGood, verifyGood } from "../../api/good";
export default {
  data() {
    return {
      goodsform: {
  ...
  methods: {
    checkGood() {
      this.checkName = this.goodsform.goodsTitle;
      this.checkForm.ids = this.$route.query.id;
      ...
      if (obj.type == "cancel") {
        ...
        verifyGood(this.checkForm).then(data => {
          if (data.data.status == 0) {
            ...
            this.$router.push({ name: "goods_checkIdx" });
          } else {
            this.$message({
              message: "审核失败",
              type: "error"
