...
<script>
import { getVerify, deleteGoods, updateGoodDown, updateGoodUp } from "../../api/good";
import { getCategory } from "../../api/category";
import { getBrandList } from "../../api/brand";
import { getGoodsListBySup } from "../../api/supplier";
...
export default {
  ...
  methods: {
    addNewGoods() {
      this.$router.push({ name: "goods_add_sup" });
    },
    alterGoods(rows) {
      this.$router.push({ name: "goods_alter_sup", query: { id: rows.id } });
    },
    viewGood(rows) {
    ...
    showAll(val) {
      getGoodsListBySup({ pageNo: Number.isInteger(val) ? val : 1, pageSize: 20 }).then(data => {
        if (data.data.status == 0) {
          this.setView(data);
        } else {
          this.$message.error("请用供应商账号登录");
        }
      });
    },
    /* 上下架 */
