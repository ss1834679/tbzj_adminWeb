<template>
  <div id="designer_alter">
    <!-- 设计师的修改 -->
    <el-card class="box-card" shadow="never">
      <div slot="header" class="clearfix">
        <div>
          <span class="title">修改设计师</span>
        </div>
      </div>
      <!-- 设计师的 表数据验证 -->
      <div class="content">
        <el-form :model="designerform" ref="designerform" :rules="designerRule" label-width="110px" class="demo-ruleForm">
          ...
          <el-form-item label="设计师售价" required prop="designerPrice">
            <precision-input
          ...
          <el-form-item label="设计师头像" required prop="designerHead">
            <upload-img
              v-if="showImg"
              @getimgArr="(imgarr)=>{this.designerform.designerHead = imgarr}"
              :imgArr="this.designerform.designerHead"
              :limit="1"
            ></upload-img>
          </el-form-item>
          <el-form-item label="设计师主图" required prop="designerImage">
            <upload-img
              v-if="showImg"
              @getimgArr="(imgarr)=>{this.designerform.designerImage = imgarr}"
              :imgArr="this.designerform.designerImage"
              :limit="1"
            ></upload-img>
          </el-form-item>
          <el-form-item label="设计师组图" required>
            <uploadimg-arr v-if="showImg" @getimgArr="(imgarr)=>{this.image = imgarr}" :imgArr="image" :limit="5"></uploadimg-arr>
          </el-form-item>
...
import { getDesignerById, updateDesigner, getDesignerCompanyAll, getDesignerStyle} from "../../api/designer";
export default {
  data() {
    return {
      designerform: {
  ...
  mounted() {
    getDesignerById(this.$route.query.id).then(data => {
      if (data.data.status == 200) {
        this.designerform = data.data.content.designer;
		if(this.designerform.designerStyle){
			this.designerform.designerStyle = this.designerform.designerStyle.split(",")
		}
        this.image = JSON.parse(this.designerform.designerPicture);
        this.styleList = this.designerform.designerStyle == "" ? [] : this.designerform.designerStyle.split(",");
        this.refreshImg();
        this.username = data.data.content.sysUser.username;
      } else {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (
            ...
            updateDesigner(this.designerform, this.image).then(data => {
              if (data.data.status == 200) {
