...
import { updateGoodBySup } from "../../api/supplier";
import { getGood, getGoodsList, getGoodsStyleByCategoryId } from "../../api/good";
        ...
        if (valid) {
          new Promise((resolve, reject) => {
            ...
            .then(() => {
              ...
              updateGoodBySup(this.goodsform).then(data => {
                if (data.data.status == 0) {
                  this.$message({
                    showClose: true,
                    message: "修改成功",
                  ...
                  this.$router.push({ name: "goods_Idx_sup" });
                } else {
                  this.$message.error(data.data.msg);
                }
              });
