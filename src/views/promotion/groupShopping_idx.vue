<template>
  <div id="groupShopping_idx">
    <!-- 拼团商品展示 -->
    <el-card class="box-card" shadow="never">
      <div slot="header" class="clearfix">
        <span class="title">拼团商品</span>
        <el-button @click="addNewGroupGoods" size="small" icon="el-icon-plus" plain>添加拼团商品</el-button>
      </div>
      <div class="content">
        ...
        <el-table :data="groupGoodsList" border style="width: 100%">
          ...
          <el-table-column label="展示拼团价格" width="120">
            <template slot-scope="scope">{{"¥"+(scope.row.goodsMarketPrice/100).toFixed(2)}}</template>
          </el-table-column>
          ...
          <el-table-column align="center" label="上下架" v-if="showSwitch" width="110">
            <template slot-scope="scope">
              <switch-com
          ...
          <el-table-column label="操作" align="center" width="155">
            <template slot-scope="scope">
              ...
              <el-button-group class="alterDel">
                <el-button type="text" @click="checkGood(scope.row)" v-if="checkButtonVisible">审核</el-button>
        ...
        <el-dialog title="商品审核" :visible.sync="checkgoodVisible">
...
import {
  ...
  deleteAssembleGoods,
  verifyAssembleGoods,
  getVerifyAssembleGoods,
  updateAssembleGoodUp,
  updateAssembleGoodDown
} from "../../api/promotion";
...
export default {
  ...
  methods: {
    addNewGroupGoods() {
      this.$router.push({ name: "groupShopping_add" });
    },
    alterGoods(rows) {
      this.$router.push({ name: "groupShopping_alter", query: { id: rows.id } });
    },
    viewGood(rows) {
      let routeUrl = this.$router.resolve({ name: "groupShopping_view", query: { id: rows.id, type: "newtag" } });
      window.open(routeUrl.href, "_blank");
    },
    /* 打开审核详情 */
    checkDetails(row) {
      /* 获取审核信息 */
      getVerifyAssembleGoods(row.id).then(data => {
    ...
    deleteGoods(rows) {
      this.$confirm("此操作将删除该商品, 是否继续?", "提示", {
        ...
        .then(() => {
          deleteAssembleGoods(rows.id).then(data => {
            if (data.data.status == 0) {
    ...
    showAll(val) {
      getAssembleGoods({ pageNo: Number.isInteger(val) ? val : 1, pageSize: 20 }).then(data => {
        if (data.data.status == 0) {
          this.setView(data);
        }
      });
    ...
    handleChangeStatus(event, id) {
      if (event) {
        updateAssembleGoodUp(id).then(data => {
        ...
        updateAssembleGoodDown(id).then(data => {
          if (data.data.status == 200) {
