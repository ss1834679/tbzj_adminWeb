            ...
            <div v-if="couponform.appoint=='2'">
              ...
              <div class="relatGoodsbox">
                <div class="relatGoodsRight">
                  <el-checkbox-group v-model="relatedList">
                    <div class="related" v-if="related.length>0">
                      <el-checkbox v-for="item in related" :label="item.id" checked :key="item.id">{{item.categoryName}}</el-checkbox>
                    </div>
                    <div v-if="appendList.length>0" class="searchgood">
...
import { addOrEditCoupon } from "../../api/promotion";
  ...
  data() {
    return {
      couponform: {
        ...
        fullMoney: null /* 满 */,
        comment: "" /* 优惠券说明 */,
        type: "3" /* 1满减券 2叠加满减券 3无门槛券（需要限制大小） */
      },
