...
import { updateAssembleGood, getAssembleGoodsById, getAssembleGoods } from "../../api/promotion";
  ...
  data() {
    return {
      goodsform: {
        ...
        goodsTitle: "" /* 商品标题 √*/,
        goodsSubtitle: "" /* 简介  √*/,
        goodsStatus: 1 /* 商品状态，0代表下架，1代表上架，2代表待审核 不管是添加还是修改都是待审核√ */,
        goodsWeight: 0 /* 商品排序值 √*/,
        brandId: "" /* 品牌ID,品牌名称 √*/,
        categoryOne: "" /* 一级分类id */,
        categoryTwo: "" /* 二级分类id */,
      ...
      goodsRules: {
        ...
        goodsPrice: [{ required: true, message: "请填写商品默认价格", trigger: "blur" }],
        ...
        if (valid) {
          new Promise((resolve, reject) => {
            ...
            .then(() => {
              ...
              updateAssembleGood(this.goodsform).then(data => {
                if (data.data.status == 0) {
                  this.$message({
                    showClose: true,
                    message: "修改成功",
