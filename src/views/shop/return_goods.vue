      ...
      <div class="content">
        ...
        <div class="status">
          <el-tabs v-model="status" type="card" @tab-click="changeStatus">
            ...
            <el-tab-pane :label="'待处理('+number.cancal+')'" name="0"></el-tab-pane>
            <el-tab-pane :label="'退货中('+number.beforePay+')'" name="1"></el-tab-pane>
            <el-tab-pane :label="'已完成('+number.beforeSend+')'" name="2"></el-tab-pane>
            <el-tab-pane :label="'已拒绝('+number.beforeReceive+')'" name="3"></el-tab-pane>
          </el-tabs>
        </div>
...
export default {
  ...
  methods: {
    addNewOrder() {},
    viewDetails(rows) {
      this.$router.push({ name: "return_Goods_Details", query: { id: rows.sn } });
    },
    closeOrder(obj) {
      if (obj.type == "open") {
