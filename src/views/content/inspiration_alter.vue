        ...
        <el-form :model="inspirationform" ref="inspirationform" :rules="inspirationRules" label-width="100px" class="demo-ruleForm">
          ...
          <el-form-item label="案列位置" required>
            <div class="searchBar">
              ...
              <div class="search_box">
                <el-select v-if="alter" v-model="district" @change="changeOrigin({level:3})" placeholder="选择区" filterable value-key="id" clearable>
                  <el-option v-for="item in districtList" :key="item.id" :label="item.criName" :value="item"></el-option>
                </el-select>
                <el-select
                  v-else
                  v-model="inspirationform.districtId"
                  disabled
                  @change="changeOrigin({level:3})"
                  placeholder="选择区"
                  filterable
                  value-key="id"
                  clearable
                >
                  <el-option v-for="item in districtList" :key="item.id" :label="item.criName" :value="item.criCode"></el-option>
                </el-select>
              </div>
            </div>
          </el-form-item>
          <el-form-item label="小区名称" required prop="villageAddressId">
            <el-select v-if="alter" v-model="inspirationform.villageAddressId" placeholder="选择小区" filterable clearable>
              <el-option v-for="item in villageList" :key="item.id" :label="item.title" :value="item.id"></el-option>
            </el-select>
            <div v-else>
              <span>{{inspirationform.villageName}}</span>
              <el-button plain size="small" @click="handlerAlter" style="margin-left:15px">修改小区</el-button>
            </div>
          </el-form-item>
          ...
          <el-divider></el-divider>
          <el-form-item v-if="showImg" label="灵感大图" required prop="bigImage">
            <upload-img
          ...
          <el-form-item v-if="showImg" label="灵感小图" required prop="thumbnailImage">
            <upload-img
...
import { insertOrUpdateInspiration, getInspirationById } from "../../api/circle";
...
export default {
  ...
  mounted() {
    if (this.$route.query.id) {
      getInspirationById(this.$route.query.id).then(data => {
        if (data.status == 200) {
          this.inspirationform = data.data.data;
          this.refreshImg();
          if (
            Array.isArray(this.inspirationform.inspirationGoods) &&
            this.inspirationform.inspirationGoods.length > 0
          ) {
            this.relatedList = this.inspirationform.inspirationGoods.map(item => {
              let obj = {};
              obj["num"] = item.num;
              obj["goodsItemId"] = item.goodsItemId;
              obj["itemTitle"] = item.goodsItem.itemTitle;
              return obj;
            });
          }

          if (this.inspirationform.decorateStyleId) {
            this.decorateArr = this.inspirationform.decorateStyleId.split("_");
          }
