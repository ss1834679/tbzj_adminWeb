<template>
  <div id="articleAlter">
    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="content">
        <el-form :model="articelform" ref="articelform" :rules="articelRules" label-width="100px" class="demo-ruleForm">
          ...
          <el-divider></el-divider>
          <el-form-item label="文章封面" required prop="image">
...
import { getChannelAll, insertOrUpdateArticle, getArticleById } from "../../api/circle";
export default {
  data() {
    return {
      articelform: {
    ...
    if (this.$route.query.id) {
      getArticleById(this.$route.query.id).then(data => {
        if (data.data.status == 0) {
          this.articelform = data.data.data;
          this.imageArr = JSON.parse(this.articelform.image);
          /* 获取该文章关联的商品 */
          if (
            Array.isArray(this.articelform.goodsRecommendModels) &&
            this.articelform.goodsRecommendModels.length > 0
          ) {
            this.related = this.articelform.goodsRecommendModels.map(item => {
              return item.goods;
            });
          }
        }
  ...
  methods: {
    ...
    appendToArticle() {
      if (this.appendGoodList.indexOf(this.searchGoodId) == -1) {
