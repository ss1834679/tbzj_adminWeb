...
<script>
import { getAdditionsInfoById } from "../../../api/trim";
export default {
  data() {
    return {
      detailsList: [
  ...
  mounted() {
    getAdditionsInfoById(this.$route.query.id).then(data => {
      if (data.data.status == 0) {
        let res = data.data.data;
        this.detailsList[0].value = res.title;
        this.detailsList[1].value = res.content;
      } else {
