      ...
      <div class="content">
        <el-form :model="adform" ref="adform" :rules="adRules" label-width="110px" class="demo-ruleForm">
          ...
          <el-form-item label="广告跳转类型">
            <el-radio-group v-model="adform.type" @change="getChange('alter')">
          ...
          <el-form-item v-if="adform.type==1" label="跳转商品">
            ...
            <span v-if="!alter" class="added">已添加商品id : {{adform.url}}</span>
          </el-form-item>
          <el-form-item v-if="adform.type==4" label="跳转团购商品">
            ...
            <span v-if="added.goodsTitle" class="added">已添加 : {{added.goodsTitle}}</span>
            <span v-if="!alter" class="added">已添加拼团商品id : {{adform.url}}</span>
          </el-form-item>
          <el-form-item v-if="adform.type==2" label="跳转文章">
            ...
            <span v-if="!alter" class="added">已添加文章id : {{adform.url}}</span>
          </el-form-item>
          <el-form-item v-if="adform.type==3" label="跳转灵感">
            ...
            <span v-if="added.title" class="added">已添加 : {{added.title}}</span>
            <span v-if="!alter" class="added">已添加灵感id : {{adform.url}}</span>
          </el-form-item>
...
import { editAdvert, getAdvertById } from "../../api/advert";
  ...
  data() {
    return {
      adform: {
        ...
        comment: "" /* 备注 */,
        isOnline: 1 /* 是否上线 */,
        type: null /* 0 外网连接 1 商品id 2 文章id 3 灵感id 4 团购商品id 10 团购列表*/
      },
      position: [
      ...
      adRules: {
        ...
        invalidTime: [{ required: true, message: "请选择广告结束使劲", trigger: "blur" }]
      }
    };
  },
  mounted() {
    if (this.$route.query.id) {
      getAdvertById(this.$route.query.id).then(data => {
        if (data.data.state == "Success") {
          this.adform = data.data.content;
        }
      });
    }
  },
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.adform.type == 10 && this.adform.url == "" && (this.adform.url = "group");
          editAdvert(this.adform).then(data => {
            if (data.data.state == "Success") {
              this.$message({
                message: "修改成功",
                type: "success"
