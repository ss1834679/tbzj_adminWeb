            ...
            <template v-if="!this.$route.query.parentId">
              <div>
                <el-cascader
                  ...
                  clearable
                ></el-cascader>
                <span style="margin-left:15px">若为一级菜单则不选择</span>
              </div>
            </template>
...
<script>
import { addmenu, getMenu, alterMenu, getMenuById } from "../../api/permission";
export default {
  data() {
    return {
      menuform: {
  ...
  mounted() {
    if (this.$route.query.id) {
      getMenuById(this.$route.query.id).then(data => {
        if (data.data.status == 200) {
          this.menuform = data.data.content;
        }
      });
    }
    /* 初始化级联列表 */
  ...
  methods: {
    submitForm() {
      if (this.$route.query.id) {
        alterMenu(this.menuform).then(data => {
          if (data.data.status == 200) {
            this.$message({
              type: "success",
              message: "菜单修改成功"
