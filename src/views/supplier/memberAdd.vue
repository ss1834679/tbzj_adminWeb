...
<script>
import { getBranchListLogin, getRoleLogin, creatUsername } from "../../api/permission";
import { insertUserSup } from "../../api/supplier_per";
export default {
  data() {
    return {
      memberForm: {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          insertUserSup(this.memberForm).then(data => {
            if (data.data.status == 200) {
              this.$router.push({ name: "member_idx_sup" });
              this.$message({
                message: "添加成功",
                type: "success"
              });
