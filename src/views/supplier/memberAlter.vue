...
<script>
import { getBranchListLogin, getRoleLogin, getUserById } from "../../api/permission";
import { updateUserSup } from "../../api/supplier_per";
export default {
  data() {
    return {
      memberForm: {
  ...
  methods: {
    /* 提交表单 */
    submitForm() {
      updateUserSup(this.roleId, this.memberForm).then(data => {
        if (data.data.status == 200) {
          this.$router.go(-1);
          this.$message({
            message: "修改成功",
