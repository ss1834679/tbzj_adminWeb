...
import { getRoleByBranch } from "../../api/permission";
import { updateRoleSup, deleteRoleSup, updateRoleStatusSup } from "../../api/supplier_per";
export default {
  data() {
    return {
      roleList: [
  ...
  methods: {
    ...
    changeStatus(id) {
      /* 改变状态 并重新请求 */
      updateRoleStatusSup(id).then(data => {
    ...
    /* 添加角色 */
    addRole() {
      this.$router.push({ name: "roleAdd_sup", query: { branchId: this.$route.query.id } });
    },
    /* 权限设置 */
    setPermission(row) {
      this.$router.push({ name: "roleSetPermission_sup", query: { id: row.id } });
    },
    /* 设置菜单 */
    setMenu(row) {
      this.$router.push({ name: "roleSetMenu_sup", query: { id: row.id } });
    },
    /* 修改名称 */
    alterName(row) {
      this.alterForm = JSON.parse(JSON.stringify(row));
    ...
    submitAlter(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          updateRoleSup(this.alterForm).then(data => {
            if (data.data.status == 200) {
              this.showAll();
              this.$message({
                message: "修改成功",
    ...
    deleteRole(row) {
      this.$confirm("此操作将删除该角色, 是否继续?", "提示", {
        ...
        .then(() => {
          deleteRoleSup(row.id).then(data => {
            if (data.data.status == 200) {
              this.showAll();
              this.$message({
                message: "删除角色成功",
