...
import { getBranchListLogin } from "../../api/permission";
import { getSupUserList, deleteUserSup, updateUserStatusSup } from "../../api/supplier_per";
...
export default {
  ...
  methods: {
    ...
    changeStatus(id) {
      /* 改变状态 并重新请求 */
      updateUserStatusSup(id).then(data => {
        if (data.data.status == 200) {
          this.$message({
            message: "修改成功",
            type: "success"
    ...
    /* 修改账户 */
    alterDetails(row) {
      this.$router.push({ name: "memberAlter_sup", query: { id: row.id } });
    },
    /* 删除账户 */
    deleteUser(row) {
      this.$confirm("此操作将删除该成员, 是否继续?", "提示", {
        ...
        .then(() => {
          deleteUserSup(row.id).then(data => {
            if (data.data.status == 200) {
              this.showAll();
              this.$message({
                message: "删除成功",
    ...
    /* 添加用户 */
    addUser() {
      this.$router.push({ name: "memberAdd_sup" });
    },
    /* 清空搜索 */
    clearSearch() {
      this.searchDepartId = "";
    ...
    handleCurrentChange(val) {
      if (this.searchValue != "" || this.searchDepartId != "") {
        //执行查询中的分页
    ...
    selectByKwd(val) {
      getSupUserList({
        pageIndex: Number.isInteger(val) ? val : 1,
        pageSize: 20,
    ...
    showAll(val) {
      getSupUserList({ pageIndex: Number.isInteger(val) ? val : 1, pageSize: 20 }).then(data => {
        if (data.data.status == 200) {
          this.setView(data);
        } else {
