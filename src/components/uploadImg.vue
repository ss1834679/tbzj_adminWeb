        ...
        if (imgArr.split(",").length > 1) {
          ...
          if (imgArr == "") {
            ...
            this.fileList = [
              {
                status: "success",
                url: imgArr.includes("http") ? imgArr : this.base_url + imgArr,
                response: { state: "Success", content: [{ name: imgArr, url: imgArr }] }
              }
            ];
          }
        }
    ...
    sendImgArr() {
      ...
      if (this.limit && this.limit == 1) {
        ...
        if (this.fileList.length == 0) {
          ...
          imgArr = imgArr + this.fileList[0].response.content[0].url;
        }
      } else {
        /* 去遍历这个组件的filelist 找到可以显示的url返回到父组件 */
        this.fileList.forEach((item, index, arr) => {
          if (index == arr.length - 1) {
            imgArr = imgArr + item.response.content[0].url;
          } else {
            imgArr = `${imgArr}${item.response.content[0].url},`;
          }
        });
